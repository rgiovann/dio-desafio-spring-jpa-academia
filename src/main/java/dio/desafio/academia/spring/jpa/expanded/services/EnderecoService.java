...
public class EnderecoService {
	...
	public EnderecoDTO findById(Long id) {
		Endereco entity = enderecoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Error. Id not found: " + id));
	...
	public EnderecoDTO update(Long id, EnderecoDTO enderecoDTO) {
			...
			Endereco entityEndereco = enderecoRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Error. Id not found: " + id));
			entityEndereco = modelMapper.map(enderecoDTO, Endereco.class);
			entityEndereco.setId(id);
